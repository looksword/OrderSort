...
os.chdir(r"E:\looksword\jupyter\OrderSort")
...
LastPourOrder = 'None'
Lastcost2Order = 'None'
all5order = pd.DataFrame(columns=['index', 'order', 'type',
                                  'startTime', 'changeTime', 'endTime',
                                  'startTimes', 'changeTimes', 'endTimes',
                                  'rate', 'WholeSet'])
all7order = pd.DataFrame(columns=['index', 'order', 'type',
                                  'startTime', 'changeTime', 'endTime',
                                  'startTimes', 'changeTimes', 'endTimes',
                                  'localPourStart', 'PourChangeTime',
                                  'rate', 'WholeSet'])
all4order = pd.DataFrame(columns=['index', 'order', 'type',
                                  'startTime', 'changeTime', 'endTime',
                                  'startTimes', 'changeTimes', 'endTimes',
                                  'rate', 'Stock'])
all2order = pd.DataFrame(columns=['index', 'order', 'type',
                                  'startTime', 'changeTime', 'endTime',
                                  'startTimes', 'changeTimes', 'endTimes', 'BagChangeTime',
                                  'rate'])
order4index = 0
order56index = 0
order7index = 0
    ...
    if (sum56 != 0 and First56) or sum7 == 0:
        ...
        for i in order_56.index:
            ...
            if total > 0:
                ...
                if test_max_order_num > stockDict[p]:
                    if orderBagStart > orderPourStart + costTime34 * 60:
                        ...
                        maxPourNum = min(int((orderBagStart - orderPourStart - costTime34 * 60) / pourtime),
                                         pourOrder[p])
                    elif (sum7 == 0) | (Not7Set == 1):
                        bagWaitTime = costTime34 * 60 + max(orderPourStart - orderBagStart, 0)
                        newpournum = test_max_order_num - stockDict[p]
                        maxPourNum = max(newpournum, pourOrder[p])
                else:
                    if orderBagStart > orderPourStart + costTime34 * 60:
                        # if order_56[i][p] > 0:
                ...
                cost_all_time = bagWaitTime + costTime56 * 60 + test_next_order_num * bagtime
                cost_rate = test_next_order_num * bagtime / cost_all_time
                # if total > max_num:
        ...
        if max_num == 0:
                    ...
                    if order_56[i][y] < stockDict[y]:
                        ...
                        all5order = all5order.append({'index': order56index,
                                                      ...
                                                      'endTimes': (orderBagStart +
                                                                   costbag[LastBagOrder][next_order] * 60 +
                                                                   sum(next_count.values()) * bagtime),
                                                      'rate': calrate(sum(next_count.values()) * bagtime,
                                                                      costbag[LastBagOrder][next_order] * 60 +
                                                                      sum(next_count.values()) * bagtime),
            ...
            all5order = all5order.append({'index': order56index,
                                          'order': next_count,
                                          'type': 5,
                                          'startTime': timestr(orderBagStart + max_waitTime),
                                          'changeTime': timestr(max_cost56 * 60),
                                          'endTime': timestr(orderBagStart + max_waitTime +
                                                             max_cost56 * 60 +
                                                             sum(next_count.values()) * bagtime),
                                          'startTimes': orderBagStart + max_waitTime,
                                          'changeTimes': (max_cost56 * 60),
                                          'endTimes': (orderBagStart + max_waitTime +
                                                       max_cost56 * 60 +
                                                       sum(next_count.values()) * bagtime),
                                          'rate': calrate(sum(next_count.values()) * bagtime, max_cost56 * 60 +
                                                          sum(next_count.values()) * bagtime),
                                          'WholeSet': sum(in_products.values())},
                                         ignore_index=True)
            order56index += 1
            ...
            if max_pournum > 0:
                all4order = all4order.append({'index': order4index,
                                              ...
                                              'endTimes': (orderPourStart +
                                                           max_cost34 * 60 +
                                                           max_pournum * pourtime),
                                              'rate': calrate(max_pournum * pourtime,
                                                              max_cost34 * 60 + max_pournum * pourtime),
                                              'Stock': nowStockDict},
                                             ignore_index=True)
                order4index += 1
                print({'index': order4index, 'type': 4,
                       'startTime': timestr(orderPourStart),
                       'order': next_pour,
                       'Stock': sum(nowStockDict.values())})
    ...
    if (sum7 != 0 and not First56) or sum56 == 0:
                    ...
                    if count[xp] > 0:
                        if localLastPourOrder == LastPourOrder:
                            ...
                            if orderPourStart < orderBagStart:
                                # costtime7pour -= min((orderBagStart - orderPourStart) / 60,
                                # costpour[localLastPourOrder][xp])
                                localpourstart = orderBagStart - min(orderBagStart - orderPourStart,
                                                                     costpour[localLastPourOrder][xp] * 60)
                        else:
                            costtime7bag += costbag[localLastBagOrder][xp]
                            costtime7pour += costpour[localLastPourOrder][xp]
        ...
        if max_num == 0:
            ...
            if sum7 < allsetnum - sum(in_products.values()):
                ...
                for i in order_7.index:
                    ...
                    for newtype in list(order_7[i].keys()):
                        if order_7[i][newtype] != 0:
                            ...
                            if newtype == LastPourOrder:
                                ...
                                if orderPourStart < orderBagStart:
                                    # costpour7pour -= min((orderBagStart - orderPourStart) / 60,
                                    # costpour[LastPourOrder][newtype])
                                    localpourstart = orderBagStart - min(orderBagStart - orderPourStart,
                                                                         costpour[LastPourOrder][newtype] * 60)
                            else:
                                costtime7bag += costbag[LastBagOrder][newtype]
                                costpour7pour += costpour[LastPourOrder][newtype]
                    ...
                    all7order = all7order.append({'index': order7index,
                                                  ...
                                                  'endTime': timestr(max(localpourstart + costpour7pour * 60,
                                                                         orderBagStart + max_cost7bag * 60) +
                                                                     sum(next_count.values()) * bagtime),
                                                  'startTimes': orderBagStart,
                                                  'changeTimes': (costtime7bag * 60),
                                                  'endTimes': (max(localpourstart + costpour7pour * 60,
                                                                   orderBagStart + max_cost7bag * 60) +
                                                               sum(next_count.values()) * bagtime),
                    ...
                    tempbagtimealpha = max(localpourstart + costpour7pour * 60,
                                           orderBagStart + max_cost7bag * 60) + sum(next_count.values()) * bagtime
            ...
            if max_localpourstart - orderPourStart > max(costpour['None'][order2alpha], cost2['None'][order2alpha]):
                ...
                if order2index == 0:
                    Last2order = 'None'
                    order2num = int((max_localpourstart - orderPourStart - costpour['None'][order2alpha] * 60) /
                                    order2time)
                else:
            ...
            for order, order_num in max_count.items():
                if len(max_count) > 1:
                    ...
                    max_order_num = min([last_order_num, order_7[max_i][order], max(order_num, pouroncenum)])
            ...
            all7order = all7order.append({'index': order7index,
                                          ...
                                          'endTime': timestr(max(max_localpourstart + max_cost7pour * 60,
                                                                 orderBagStart + max_cost7bag * 60) +
                                                             sum(next_count.values()) * bagtime),
                                          'startTimes': orderBagStart,
                                          'changeTimes': (max_cost7bag * 60),
                                          'endTimes': (max(max_localpourstart + max_cost7pour * 60,
                                                           orderBagStart + max_cost7bag * 60) +
                                                       sum(next_count.values()) * bagtime),
                                          'localPourStart': max_localpourstart,
                                          'PourChangeTime': (max_cost7pour * 60),
                                          'rate': calrate(sum(next_count.values()) * bagtime,
                                                          max_cost7bag * 60 + sum(next_count.values()) * bagtime),
                                          'WholeSet': sum(in_products.values())},
                                         ignore_index=True)
            order7index += 1
            print({'index': order7index,
            ...
            tempbagtimealpha = max(max_localpourstart + max_cost7pour * 60,
                                   orderBagStart + max_cost7bag * 60) + sum(next_count.values()) * bagtime
            orderBagStart = tempbagtimealpha
            orderPourStart = orderBagStart
            for order, order_num in next_count.items():
...
if NotExplode:
    Summary = pd.DataFrame(columns=['包装线制作用时', '包装线换型换料用时', '包装线总用时', '包装线效率',
                                    '灌装线制作用时', '灌装线换型换料用时', '灌装线总用时', '灌装线效率'])
    Summary = Summary.append({'包装线制作用时': "%.2f" % (allbagttime/60),
                              '包装线换型换料用时': "%.2f" % (allbagctime/60),
                              '包装线总用时': "%.2f" % (allbagtime/60),
                              '包装线效率': calrate(allbagttime, allbagtime),
...
colors = {'包装':'rgb(0,200,0)','灌装':'rgb(0,100,100)','灌包':'rgb(255,205,0)','换型换料':'rgb(200,0,0)'}
