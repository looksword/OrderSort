...
if __name__ == '__main__':
                ...
                if total > 0:
                    ...
                    if test_max_order_num > stockDict[p]:
                        if orderBagStart > orderPourStart + costTime34 * 60:
                            ...
                            max_pournum = max(newpournum, pourOrder[p])
                    else:
                        if orderBagStart > orderPourStart + costTime34 * 60:
                            # if order_56[i][p] > 0:
                    ...
                    cost_all_time = bagWaitTime * 60 + costTime56 * 60 + test_next_order_num * bagtime
                    cost_rate = test_next_order_num * bagtime / ()
                    # if total > max_num:
    ...
    if NotExplode:
        Summary = pd.DataFrame(columns=['包装线制作用时', '包装线换型换料用时', '包装线总用时','包装线效率', '灌装线制作用时', '灌装线换型换料用时', '灌装线总用时','灌装线效率'])
